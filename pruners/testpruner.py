...
def _count_differences_between_masks(mask1, mask2):
    n_same = 0
    n = 0
...
class testpruner(Algorithm):
    def __init__(
            ...
            sigma1=0.05,
            # initial factor value for sigma1
            alpha_i_sigma1=1.0,
            # final factor value for sigma1
            alpha_f_sigma1=1.0,
            # start step of annealing for sigma1
            anneal_start_sigma1=None,
            # end step of annealing for sigma1
            ...
            # interval for logging how mask corresponds to the final ratio changes during the gradual pruning stage
            mask_change_log_interval=None,
        ):

        self.iw_final_ratio_mask = None
        self.current_final_ratio_mask = None
        ...
        self.anneal_start_sigma1 = anneal_start_sigma1 if anneal_start_sigma1 is not None else 0
        self.anneal_end_sigma1 = anneal_end_sigma1 if anneal_end_sigma1 is not None else pruning_start
        ...
        if mask_ind and ratio < 1.0:
                if train_step_index == self.pruning_end:
                    ...
                    if self.final_fixed_mask is not None:
                        ...
                        mask_threshold = 0.0
                    # when load from a checkpoint that is >= pruning_end, then the final_fixed_mask is None
                    else:
                        mask_threshold, mask = self.mask_with_threshold(model, ratio)
                        self.final_fixed_mask = mask
                else:
    ...
    def apply(self, event, state, logger):
        if event == Event.FIT_START:
            ...
            # perform gradient clipping during the final warmup stage if clipping_threshold is not None
            ...
            if self.mask_change_log_interval is not None and state.timestamp.batch.value <= self.pruning_end:
                if state.timestamp.batch.value == self.pruning_start:
                    ...
                    self.iw_final_ratio_mask = self.create_mask(state.model, mask_threshold, is_dict)
                elif state.timestamp.batch.value > self.pruning_start and state.timestamp.batch.value % self.mask_change_log_interval == 0:
                    mask_threshold, is_dict = self.calculate_mask_threshold(state.model, self.final_ratio)
                    updated_final_ratio_mask = self.create_mask(state.model, mask_threshold, is_dict)
                    n_diff_wrt_iwm = _count_differences_between_masks(self.iw_final_ratio_mask, updated_final_ratio_mask)
                    if self.current_final_ratio_mask is not None:
                        n_diff_wrt_lm = _count_differences_between_masks(self.current_final_ratio_mask, updated_final_ratio_mask)
                        logger.log_metrics({"n_diff_wrt_last_mask": int(n_diff_wrt_lm)})
                    self.current_final_ratio_mask = updated_final_ratio_mask
                    logger.log_metrics({"n_diff_wrt_mask_after_initial_warmup": int(n_diff_wrt_iwm)})
            ...
            # log the parameter's magnitude statistics before the pruning starts
